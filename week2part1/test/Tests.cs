...
public record HammingDistanceTestCase(uint Number, uint OtherNumber, int ExpectedDistance);
...
public class Tests
{
    ...
    {
        // Settings
        uint seed = 0; // The value of seed doesn't affect the COUNT, only the values.
        int totalBits = 32;
    ...
    {
        // Sample known values
        var knownValues = new List<(uint number, int distance)>
        {
            (6, 2),
            (7, 1),
            (4, 3)
